...
public class CircleColorChanger : MonoBehaviour
{
    ...
    public float distancefromhome = 2f;
        ...
        if (circlePrefab != null && targetPrefab != null)
        {
            ...
            if (assessmentRounds == 1)
            {
                ...
                for (int i = 0; i < numberOfTargets; i++)
                {
                    ...
                    float angleStep = (Mathf.PI) / (numberOfTargets - 1); // Angle increment for a semicircle
                    

                    //check the code
                    //float x = distanceFromHome * Mathf.Cos(angle);
                    //float y = distanceFromHome * Mathf.Sin(angle);
    ...
    IEnumerator HideTextAfterDelay()
    {
        ...
        //if (assessmentoverText != null)
        //{
        //    yield return new WaitForSeconds(textDisplayDelay); // Additional delay before hiding "Assessment Over" text
        //    assessmentoverText.gameObject.SetActive(false); // Disable the "Assessment Over" text
        //}
    }
